...
class IQR: 
    def __init__(self, df: DF) -> None: 
        self.df_numerico = df.ColumnaNumerica
    
    def cuartiles(self, col: str, max_q: float=0.75, min_q: float=0.25) -> tuple: 
...
class OutlierCleaner: 
    def __init__(self, df: DF) -> None:
        self.df_limpieza = df.ColumnaNumerica
        self.IQR_object = IQR(df=df)
    
    def winsorizacion(self, col: str, max_q: float=0.75, min_q: float=0.25) -> pl.DataFrame:
    ...
    def eliminacion(self, col:str, max_q: float=0.75, min_q: float=0.25) -> pl.DataFrame: 
...
class PipelineLimpiezaOutliers: 
    def __init__(self, df: pl.DataFrame) -> None:
        self.df = df
        self.df_numerico = DF(df=df)
        self.estrategia_eliminacion = OutlierCleaner(df=self.df_numerico)
    
    def PipelineLimpiezaOutlier(self, col: str, estrategia: EstrategiasOutlier, max_q: float=0.75, min_q: float=0.25) -> pl.DataFrame:
        match estrategia: 
