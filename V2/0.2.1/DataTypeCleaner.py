...
class DFWrapper: 
    ...
    @property
    def ColumnaFecha(self) -> None: 
        return ColumnAnalyzer(df=self.df).columna_fecha()
...
class FormatoDeFecha:
    def __init__(self, df: DFWrapper) -> None: 
        self.validacion = ValidacionesDinamicas(df=df.df)
        self.validacion.dataframe_vacio()
        self.df_fecha = df.ColumnaFecha
    
    def Año_Mes_Dia(self, col:str) -> pl.DataFrame: 
        self.validacion.columna_fecha(col=col)
        try:
...
class FormatoDeFechaTZSinZona: 
    def __init__(self, df: DFWrapper) -> None: 
        self.validacion = ValidacionesDinamicas(df=df.df)
        self.validacion.dataframe_vacio()
        self.df_fecha_TZ = df.ColumnaFecha
